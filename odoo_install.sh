...
# Worker Count for Odoo, this is the number of workers that will be started by default
WORKER_COUNT=4
...
if [ $IS_ENTERPRISE = "True" ]; then
    ...
    GITHUB_RESPONSE=$(sudo git clone --depth 1 --branch $OE_VERSION https://www.github.com/astasys/enterprise "$OE_HOME/enterprise/addons" 2>&1)
...
sudo su root -c "echo 'source /$OE_USER/venv/bin/activate' >> $OE_HOME_EXT/start.sh"
...
User=$OE_USER
Group=$OE_USER
ExecStart=/$OE_USER/venv/bin/python ${OE_HOME_EXT}/odoo-bin -c /etc/${OE_CONFIG}.conf
